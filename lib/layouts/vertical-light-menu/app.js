var App = function() {
    ...
    var toggleFunction = {
        sidebar: function($recentSubmenu) {
            /* NutriNext: lógica unificada en nutrinext-dashboard-sidebar.js (evita doble toggle en móvil) */
            if (document.body.dataset.nnSidebarBound === '1') {
                return;
            }
            ...
            togglethemeEl.addEventListener('click', function() {
                ...
                if (parseObj.settings.layout.darkMode) {
                    ...
                    if (!getUpdatedParseObject.settings.layout.darkMode) {
                        ...
                        var baseUrl = (typeof window.NutriNextBaseUrl !== 'undefined' && window.NutriNextBaseUrl) ? window.NutriNextBaseUrl : '';
                        var logoLight = ifStarterKit ? (baseUrl + 'lib/logo/logo-horizontal.png') : (getUpdatedParseObject.settings.layout.logo.lightLogo || (baseUrl + 'lib/logo/logo-horizontal.png'));
                        if (document.querySelector('.navbar-logo')) {
                            document.querySelector('.navbar-logo').setAttribute('src', (logoLight.indexOf('/') === 0 || logoLight.indexOf('http') === 0) ? logoLight : (baseUrl + (logoLight || '').replace(/^\.\.\//, '')));
                        }
                    }
                    ...
                    if (getUpdatedParseObject.settings.layout.darkMode) {
                        ...
                        ifStarterKit = document.body.getAttribute('page') === 'starter-pack' ? true : false;
                        var baseUrl = (typeof window.NutriNextBaseUrl !== 'undefined' && window.NutriNextBaseUrl) ? window.NutriNextBaseUrl : '';
                        var logoDark = ifStarterKit ? (baseUrl + 'lib/logo/logo-horizontal.png') : (getUpdatedParseObject.settings.layout.logo.darkLogo || (baseUrl + 'lib/logo/logo-horizontal.png'));
                        if (document.querySelector('.navbar-logo')) {
                            document.querySelector('.navbar-logo').setAttribute('src', (logoDark.indexOf('/') === 0 || logoDark.indexOf('http') === 0) ? logoDark : (baseUrl + (logoDark || '').replace(/^\.\.\//, '')));
                        }
                    }
    ...
    var inBuiltfunctionality = {
        mainCatActivateScroll: function() {
            var menuEl = document.querySelector('.menu-categories');
            if (!menuEl) {
                return;
            }
            /* En móvil / touch el PS bloquea scroll del menú del entrenador */
            if (window.innerWidth <= MediaSize.md || window.matchMedia('(pointer: coarse)').matches) {
                return;
            }
        ...
        preventScrollBody: function() {
            var nonScrollableElement = document.querySelectorAll('.user-profile-dropdown .dropdown-menu, .notification-dropdown .dropdown-menu,  .language-dropdown .dropdown-menu')
