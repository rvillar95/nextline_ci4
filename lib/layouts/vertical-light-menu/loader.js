    ...
    var settingsObject = {
        ...
        settings: {
            layout: {
                ...
                boxed: false,
                logo: {
                    darkLogo: baseUrl + 'lib/logo/logo-grande-sin-margen.png',
                    lightLogo: baseUrl + 'lib/logo/logo-grande-sin-margen.png'
                }
            }
        },
        reset: false
    ...
    if (corkThemeObject.settings.layout.darkMode) {
        ...
        if (getParseObject.settings.layout.darkMode) {
            ...
            if (document.querySelector('.navbar-logo')) {
                var src = ifStarterKit ? (baseUrl + 'lib/logo/logo-grande-sin-margen.png') : (getParseObject.settings.layout.logo.darkLogo || (baseUrl + 'lib/logo/logo-grande-sin-margen.png'));
                if (src.indexOf('/') === 0 || src.indexOf('http') === 0) { document.querySelector('.navbar-logo').setAttribute('src', src); }
                else { document.querySelector('.navbar-logo').setAttribute('src', baseUrl + src.replace(/^\.\.\//, '')); }
            }
        }
    } else {
        localStorage.setItem("theme", JSON.stringify(corkThemeObject));
        ...
        if (!getParseObject.settings.layout.darkMode) {
            ...
            if (document.querySelector('.navbar-logo')) {
                var src = ifStarterKit ? (baseUrl + 'src/assets/img/logo2.svg') : (getParseObject.settings.layout.logo.lightLogo || (baseUrl + 'lib/logo/logo-grande-sin-margen.png'));
                if (!src) src = baseUrl + 'lib/logo/logo-grande-sin-margen.png';
                if (src.indexOf('/') === 0 || src.indexOf('http') === 0) { document.querySelector('.navbar-logo').setAttribute('src', src); }
                else { document.querySelector('.navbar-logo').setAttribute('src', baseUrl + src.replace(/^\.\.\//, '')); }
            }
        }
