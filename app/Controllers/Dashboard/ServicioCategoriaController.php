    ...
    public function getServicioCategoria()
    {
        ...
        foreach ($books as $r) {
            $data[] = array(
                ...
                $r->icono ? '<i class="' . $r->icono . '" style="color: ' . $r->color . '; font-size: 20px;"></i>' : 'Sin icono',
                $r->color ? '<span style="background-color: ' . $r->color . '; color: white; padding: 2px 8px; border-radius: 4px;">' . $r->color . '</span>' : 'Sin color',
                $r->total_servicios ?? 0,
                $r->orden,
